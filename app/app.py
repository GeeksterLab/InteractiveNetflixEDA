...
def select_enriched_file():
    ...
    if not file_dates:
        raise FileNotFoundError(
            f"Aucun fichier enriched_netflix_history trouvé dans : {directory}"
        )
...
def get_top_20_tv_shows_figure(df):
    ...
    top_tv = tv_shows["Title"].value_counts().nlargest(20).reset_index()
    top_tv.columns = ["Title", "count"]
    fig = px.bar(
        top_tv,
        x="Title",
        y="count",
        title="Top 20 des séries les plus regardées",
        labels={"Title": "Série", "count": "Nombre de visionnages"}
    )
    return fig
